...
class ScheduleSubmissionObserver extends Observer
{
    ...
    public static function admitting($scheduleSubmission)
    {
        $attendance = Attendance::where(
            ...
            ->where('attendance_type_id', $scheduleSubmission->attendance_type_id)
            ->first();

        if (is_null($attendance)) {
            $attendance = new Attendance;
            $attendance->employee_id = $scheduleSubmission->employee_id;
            $attendance->branch_id = $scheduleSubmission->branch_id;
            $attendance->attendance_type_id = $scheduleSubmission->attendance_type_id;
        }
