                    ...
                    <template>
                        <bsb-form-group v-if="!state.form.table.loading">
                            ...
                            <bsb-table-responsive class="p-1 mt-3">
                                <bsb-table :hover="true" :bordered="true">
                                    <thead class="thead-light">
                                        <th>#</th>
                                        <th>Nama Karyawan</th>
                                        <th v-for="(dates, i) in state.form['dates']" :key="i">
                                            {{ dates }}
                                        </th>
                                    </thead>
                                    <tbody>
                                        <tr v-for="(employee, i) in resources['employees']" :key="i">
                                            <td>{{ i + 1 }}</td>
                                            <td>{{ employee['full_name'] }}</td>
                                            <th v-for="(date, j) in state.form['dates']" :key="j">
